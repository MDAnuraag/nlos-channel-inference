"""Experiment runner for discrimination / leakage analysis with a multi-state operator."""
...
class MultiStateDiscriminationConfig:
    """Configuration for one multi-state discrimination experiment."""
...
@dataclass(frozen=True)
class MultiStateDiscriminationExperimentResult:
    """Outputs from one multi-state discrimination experiment."""
...
def run_multi_state_discrimination_experiment(
    *,
...
) -> MultiStateDiscriminationExperimentResult:
    """Run a discrimination / leakage experiment using a multi-state operator."""
    ...
    report = {
        "experiment_type": "run_multi_state_discrimination",
        "mode": config.mode,
    ...
    save_json(report, run_dir / "reports" / "multi_state_discrimination_report.json")

    manifest = {
        "artifact_type": "multi_state_discrimination",
    ...
    return MultiStateDiscriminationExperimentResult(
        result=result,
        run_dir=run_dir,
        report=report,
    )