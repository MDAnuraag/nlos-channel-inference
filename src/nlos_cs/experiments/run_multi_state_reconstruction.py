"""Experiment runner for reconstruction with a multi-state operator."""
...
class MultiStateReconstructionConfig:
    """Configuration for one multi-state reconstruction experiment."""
    ...
    save_per_state_measurement_blocks: bool = True


@dataclass(frozen=True)
class MultiStateReconstructionExperimentResult:
    """Outputs from one multi-state reconstruction experiment."""
...
def _per_state_measurement_blocks(
    y: OperatorArray,
    row_slices: dict[str, slice],
) -> dict[str, OperatorArray]:
    """Split one measurement vector into per-state row blocks."""
    return {state_id: y[sl] for state_id, sl in row_slices.items()}


def run_multi_state_reconstruction_experiment(
    *,
...
) -> MultiStateReconstructionExperimentResult:
    """Run one reconstruction experiment using a multi-state operator."""
    ...
    if row_slices and config.save_per_state_measurement_blocks:
        blocks = _per_state_measurement_blocks(y_use, row_slices)
        for state_id, block in blocks.items():
            extra_arrays[f"y_block_{state_id}"] = block
        if y_true is not None:
            true_blocks = _per_state_measurement_blocks(y_true, row_slices)
            for state_id, block in true_blocks.items():
                extra_arrays[f"y_true_block_{state_id}"] = block
    ...
    report = {
        "experiment_type": "run_multi_state_reconstruction",
        "solver_name": recon.solver_name,
        "lambda_value": recon.lambda_value,
        "objective_value": recon.objective_value,
        "residual_norm": recon.residual_norm,
    ...
    save_json(report, run_dir / "reports" / "multi_state_reconstruction_report.json")

    return MultiStateReconstructionExperimentResult(
