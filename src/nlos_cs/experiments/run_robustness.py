"""Experiment runner for robustness sweeps under repeated perturbations.

Current scope
-------------
- AWGN robustness sweeps over:
  - true position
  - noise level
  - trial index
...
Design
------
This is an experiment/orchestration layer. It should not contain solver
implementations or operator construction logic.
"""
...
class RobustnessConfig:
    """Configuration for one robustness sweep."""
    ...
    save_peak_count_cubes: bool = True


@dataclass(frozen=True)
class RobustnessExperimentResult:
    """Outputs from one robustness experiment."""
...
def run_awgn_robustness_experiment(
    *,
    ...
    """Run an AWGN robustness sweep.

    For each true position i and noise level alpha:
        - generate n_trials noisy versions of y_true = A[:, i]
        - reconstruct x_hat
        - record:
            success rate
    ...
    peak_count_cube = (
        ...
        if config.save_peak_count_cubes
        else None
    )
    ...
    save_json(report, run_dir / "reports" / "robustness_report.json")
